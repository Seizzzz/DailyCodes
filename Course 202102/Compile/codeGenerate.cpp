...
map<string,string> mapSubPrgm2HeadFile;//过程、函数到C程序头文件的映射  
map<string,bool> mapHeadFileShow;//头文件是否出现的映射

//全局常量
vector<string> vecConstId;//常量标识符  
vector<string> vecConstType;//常量类型,"integer","real","char","boolean"  
vector<string> vecConstValue;//常量值,不论是哪种类型，均用string表示
//全局变量
vector<string> vecVariantId;//变量标识符  
vector<string> vecVariantType;//变量类型,"integer","real","char","boolean"  
vector<vector<int>> vecArraySize;//各数组变量各维的大小
...
struct subproDec{
	...
	vector<string> vecParmType;//参数类型,"integer","real","char","boolean"
	subproDec(){
		vecParmId.clear();
		vecParmIsRef.clear();
		vecParmType.clear();
...
vector<subproDec> vecSubproDec;//子程序接口列表
...
struct subproDef{
	...
	vector<pair<string,int>> vecStatement;//pair的first是语句本身，pair的second控制缩进，即语句前的制表符个数
};
//子程序定义列表
vector<subproDef> vecSubproDef;

string subMainFunctionDeclaration;//原PASCAL主程序对应到C程序的声明
vector<pair<string, int>> vecStatement;//主程序语句列表
...
void inputConstList(vector<_Constant *>&, vector<string>&, vector<string>&, vector<string>&, _SymbolTable*);//获取常数列表
void inputVariantList(vector<_Variant *>, vector<string>&, vector<string>&, vector<vector<int>>&, _SymbolTable*);//获取变量列表
void inputSubproDecList();//获取子程序声明列表
void inputFunctionCall(_FunctionCall*, string&, int mode=0);//获取函数调用
int inputExpression(_Expression*, string&, int mode=0, bool isReferedActualPara=false);//获取表达式
void inputVariantRef(_VariantReference*, string&, int mode=0, bool isReferedActualPara=false);//获取变量引用
void inputStatementList(_Statement*, vector<pair<string,int>>&, int retract, int flag=0);//获取语句列表
void inputSubproDef(_FunctionDefinition*);//获取子程序定义
void inputSubproDefList(_SubProgram*);//获取子程序定义列表
...
bool checkAndInputLibrarySubprogram(_ProcedureCall*, vector<pair<string, int>>&, int retract); //检查并获取库程序

void outputHeadFileList();//输出头文件
void outputConstList(vector<string> &constIdList, vector<string>&, vector<string>&, int retract = 0);//输出常数列表
void outputVariantList(vector<string>&, vector<string>&, vector<vector<int>>&, int retract = 0);//输出变量列表
void outputSubproDec(const subproDec&, int flag = 0);//输出子程序声明
...
void codeGenerate(_Program *ASTRoot, string outFile) {
	...
	inputConstList(ASTRoot->subProgram->constList, vecConstId, vecConstType, vecConstValue, codeGenerateCurrentSymbolTable);//全局常量/主程序常量
	inputVariantList(ASTRoot->subProgram->variantList, vecVariantId, vecVariantType, vecArraySize, codeGenerateCurrentSymbolTable);//全局变量/主程序变量
	inputSubproDefList(ASTRoot->subProgram);//子程序定义列表
	codeGenerateCurrentSymbolTable = mainSymbolTable;//定位到主符号表
	inputSubMainFunction(ASTRoot);//原PASCAL主程序对应C的程序头和语句体

	outputHeadFileList();
	outputConstList(vecConstId, vecConstType, vecConstValue);
	outputVariantList(vecVariantId, vecVariantType, vecArraySize);
	outputSubproDecList();
	outputMain(ASTRoot);
	outputSubproDefList();
...
void beforeCodeGenerate(){
	...
	mapSubPrgm2HeadFile["read"]="stdio.h";
	mapSubPrgm2HeadFile["write"]="stdio.h";
	mapSubPrgm2HeadFile["writeln"]="stdio.h";
	mapHeadFileShow["stdio.h"]=false;
}
...
void inputHeadFileList(string subProgramId){
	if (mapSubPrgm2HeadFile.count(subProgramId))
		mapHeadFileShow[mapSubPrgm2HeadFile[subProgramId]] = true;
}
...
void inputSubproDef(_FunctionDefinition* functionDefinitionNode){
	...
	inputConstList(functionDefinitionNode->constList, tmp.vecConstId, tmp.vecConstType, tmp.vecConstValue, record->subSymbolTable);
	inputVariantList(functionDefinitionNode->variantList, tmp.vecVariantId, tmp.vecVariantType, tmp.vecArraySize, record->subSymbolTable);
	inputStatementList(functionDefinitionNode->compound, tmp.vecStatement, 1, 1);
	vecSubproDef.push_back(tmp);
}
...
void inputSubMainFunction(_Program* ASTRoot) {
	...
	inputStatementList(ASTRoot->subProgram->compound, vecStatement, 1, 1);
}
...
void outputHeadFileList() {
	if (!mapHeadFileShow.empty())
		fout << "//Head files" << endl;
	else return;
	
	for (auto iter : mapHeadFileShow)
	{
		if(iter.second){
			fout << "#include<" << iter.first << ">" << endl;
			iter.second = false;
		}
	}
...
void outputSubproDec(const subproDec& tmp, int flag) {//flag==0时表示接口声明,flag==1时表示定义时的子程序头
...
void outputSubproDecList() {
	if (vecSubproDec.empty()) return;
	fout << "//Subprogram declaration" << endl;

	fout << subMainFunctionDeclaration << ";" << endl; //main
	for (const auto iter : vecSubproDec)
		outputSubproDec(iter, 0);
...
void outputSubproDefList() {
	vector<subproDef> &vec = vecSubproDef;
	int n = int(vecSubproDef.size());
	...
	fout << "{" << endl;
	outputStatementList(vecStatement);
	fout << "}" << endl << endl;
	for (int i = 0; i < n; i++) {
		outputSubproDec(vecSubproDec[i], 1);
		fout << "{" << endl;
		outputConstList(vec[i].vecConstId, vec[i].vecConstType, vec[i].vecConstValue, 1);
		outputVariantList(vec[i].vecVariantId, vec[i].vecVariantType, vec[i].vecArraySize, 1);
		outputStatementList(vec[i].vecStatement);
		fout << "}" << endl << endl;
	}
